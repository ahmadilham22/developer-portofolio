...
export default function ProjectsPage() {
  const allProjects = [...mockData.projects, ...(mockData.archivedProjects || [])]
  ...
  return (
    <main className="w-full bg-background">
      ...
      <section className="py-20 px-6 max-w-7xl mx-auto">
        ...
        {mockData.archivedProjects && mockData.archivedProjects.length > 0 && (
          <div>
            <h2 className="text-2xl md:text-3xl font-bold mb-8">Archived Projects</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 auto-rows-[350px]">
              {mockData.archivedProjects.map((project) => (
                <ProjectCard
                  key={project.id}
                  project={project}
                  size="medium"
